...
from atari_wrapper import wrap_deepmind, make_atari as wrap_atari
from utils import load_expert_data_atari
...
if __name__ == "__main__":
    ...
    parser.add_argument('--expert-trajectories-path', type=str, default=None,
                        help="Path to expert trajectories stored as a .pkl file.")
...
optimizer = optim.Adam(list(agent.network.parameters()) + 
                        list(agent.actor.parameters()) +
                        list(agent.critic.parameters()), lr=args.learning_rate, eps=1e-5)
optimizer_d = optim.Adam(agent.discriminator.parameters())
if args.anneal_lr:
...
if GAIL:
    ...
    expert_obs, expert_actions = load_expert_data_atari('/home/usman/repos/\
    rl_zoo/rl-baselines-zoo/ExpertData/ppo2_BreakoutNoFrameskip-v4', num_of_trajs=2)
    ...
    for step in range(0, args.num_steps):
        ...
        for info in infos:
            if 'episode' in info.keys():
                ...
                reward_log[global_step] = info['episode']['r']
                break

    if GAIL:
        optimizer_d.zero_grad()    
        e_preds = agent.discriminate_fn(expert_obs, expert_actions)
        g_preds = agent.discriminate_fn(obs, actions)
        disc_loss = disc_loss_fn(e_preds, torch.zeros(*e_preds.shape)) + \
                    disc_loss_fn(g_preds, torch.ones(*g_preds.shape))
        disc_loss.backward()
        optimizer_d.step()

        rewards = agent.evaluate_reward(obs, actions).to(device)
        print(rewards.shape)
